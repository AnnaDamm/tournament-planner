import type { en } from './en'

type Translation = { [Key in keyof typeof en]: string }

export const de: Translation = {
  tournament: 'TURNIER',
  players: 'Spieler',
  teams: 'Teams',
  table: 'Tabelle',
  rounds: 'Runden',
  settings: 'Turniereinstellungen',
  back: 'Zurück',
  menu: 'Menü',
  expandNavigation: 'Turniernavigation ausklappen',
  collapseNavigation: 'Turniernavigation einklappen',
  showHeaderMenu: 'Oberes Menü einblenden',
  actions: 'Aktionen',
  documentation: 'Dokumentation',
  add: 'Spieler hinzufügen',
  addTeam: 'Teams hinzufügen',
  participants: 'TEILNEHMER',
  ranking: 'RANGLISTE',
  schedule: 'SPIELPLAN',
  create: 'Runde erstellen',
  calculatePairings: 'Paarungen berechnen',
  startRound: 'Runde starten',
  all: 'Alle Spieler',
  allTeams: 'Alle Teams',
  registered: 'registriert',
  ready: 'Bereit für den ersten Aufschlag?',
  intro: 'Füge alle Teilnehmenden hinzu, um dein Turnier zu starten.',
  noRounds: 'Noch keine Runden',
  firstRound: 'Erstelle die erste Runde, sobald alle Teilnehmenden bereit sind.',
  wins: 'SIEGE',
  losses: 'NIEDERLAGEN',
  games: 'SPIELE',
  fewerGames: 'Weniger Spiele als die aktuelle Höchstzahl',
  sets: 'SÄTZE',
  points: 'PUNKTE',
  difference: 'PUNKTE-DIFFERENZ',
  position: 'POS.',
  onePerLine: 'Ein Name pro Zeile · Strg/⌘ + Enter zum Hinzufügen',
  cancel: 'Abbrechen',
  save: 'Speichern',
  addButton: 'Hinzufügen',
  oddTitle: 'Ungerade Zahl an Teilnehmenden',
  oddQuestion: 'Wer erhält den automatischen Sieg?',
  automatic: 'Automatisch nach Tabellenplatz',
  choose: 'Teilnehmenden auswählen',
  bye: 'Freilos',
  match: 'Begegnungen',
  round: 'RUNDE',
  currentRound: 'Aktuelle Runde',
  type: 'Teilnehmendentyp',
  typeHelp: 'Wähle, ob dein Turnier für einzelne Spieler oder Teams organisiert wird.',
  tournamentName: 'Turniername',
  tournamentNameHelp: 'Der Name wird im Kopfbereich angezeigt und exportiert.',
  exportData: 'JSON exportieren',
  importData: 'JSON importieren',
  importError: 'Diese JSON-Datei konnte nicht importiert werden.',
  courts: 'Spielfelder',
  courtsHelp: 'Anzahl der Begegnungen, die parallel laufen können.',
  court: 'Feld',
  start: 'Start',
  defaultWinningGames: 'Gewinnsätze im Turnier',
  defaultWinningGamesHelp: 'Standardanzahl der Sätze, die gewonnen werden müssen.',
  defaultSetPoints: 'Standardpunkte pro Satz',
  defaultSetPointsHelp: 'Standardpunktzahl für einen Satz und ein Freilos.',
  roundSettings: 'Rundeneinstellungen',
  running: 'Läuft gerade',
  winningGames: 'Gewinnsätze',
  deleteAll: 'Lösche alles',
  deleteEverything: 'Alles löschen?',
  deleteDescription: 'Alle Teilnehmenden, Runden und Ergebnisse werden entfernt.',
  selected: 'ausgewählt',
  editName: 'Name bearbeiten',
  unknown: 'Unbekannt',
  delete: 'Löschen',
  clickToRename: 'Klicken zum Umbenennen',
  deleteRound: 'Runde löschen',
  duplicateWarning: 'Warnung: Zwei oder mehr Teilnehmende haben den gleichen Namen.',
  edit: 'Bearbeiten',
  done: 'Fertig',
  fillMore: 'Weitere befüllen',
  notYetKnown: '<noch nicht bekannt>',
  history: 'Spielhistorie',
  historyEmpty: 'Noch keine Spiele gespielt.',
  comparison: 'Spielervergleich',
  comparisonDescription: 'Turnierstatistiken im Vergleich',
  comparePlayers: 'Spieler vergleichen',
  comparisonViews: 'Vergleichsansicht',
  currentStats: 'Aktuell',
  beforeSelectedRound: 'Vor der Runde',
  statistics: 'Statistik',
  headToHead: 'Direktduelle',
  noHeadToHead: 'Bisher kein direktes Duell.',
  playedMatches: 'Gespielte Begegnungen',
  record: 'Bilanz',
  positionBeforeRound: 'Pos. davor',
  positionAfterRound: 'Pos. danach',
  opponent: 'GEGNER',
  score: 'ERGEBNIS',
  result: 'RESULTAT',
  win: 'GEWONNEN',
  loss: 'VERLOREN',
  pending: 'OFFEN',
  close: 'Schließen',
  withdraw: 'Teilnahme aufgeben',
  undoWithdrawal: 'Aufgabe rückgängig machen',
  withdrawn: 'aufgegeben',
  reroll: 'Paarungen neu berechnen',
  rerollBye: 'Bye neu berechnen',
  shuffle: 'Mischen',
  scheduledStart: 'Geplanter Start',
  scheduledStartHelp: 'Die erste Runde startet zu diesem lokalen Termin automatisch.',
  expectedDuration: 'Erwartete Spieldauer (Minuten)',
  expectedDurationHelp: 'Wird zur Berechnung der erwarteten Startzeiten verwendet.',
  breakBetweenMatches: 'Pause zwischen Spielen (Minuten)',
  breakBetweenMatchesHelp: 'Pause vor dem Start des nächsten Spieldurchlaufs.',
  expectedStart: 'Erwarteter Start',
  startedAt: 'Gestartet um',
  pairingsPending: 'Paarungen noch nicht berechnet.',
  playerSearch: 'Spieler suchen',
  clearSearch: 'Suche leeren',
  nextMatch: 'Nächstes Spiel',
  noNextMatch: 'Kein kommendes Spiel',
  previousFinding: 'Vorheriger Treffer',
  nextFinding: 'Nächster Treffer',
  liveView: 'LIVE-ANSICHT',
  shareTournament: 'Turnier teilen',
  scanToView: 'Scannen, um live zuzuschauen',
  shareHelp: 'Spieler sehen nach dem Scan Tabelle, Runden und Teilnehmende live.',
  viewerAddress: 'Zuschaueradresse',
  viewerQrCode: 'QR-Code für die Live-Ansicht des Turniers',
  qrError: 'Der QR-Code konnte nicht erzeugt werden.',
  liveConnectionOnline: 'Live-Verbindung aktiv',
  liveConnectionOffline: 'Nicht live — erneuter Verbindungsversuch',
  enableReadOnly: 'Read-only-Ansicht aktivieren',
  enableFocusMode: 'Große Ansicht aktivieren',
  disableFocusMode: 'Große Ansicht deaktivieren',
  enableRotationMode: 'Automatischen Ansichtswechsel aktivieren',
  disableRotationMode: 'Automatischen Ansichtswechsel deaktivieren',
  skipToContent: 'Zum Hauptinhalt springen',
  tournamentNavigation: 'Turniernavigation',
  footerLinks: 'Projektlinks',
  github: 'GitHub',
  reportIssue: 'Fehler melden / Verbesserungen vorschlagen',
  developerAuthor: 'Entwickelt von: Anna Damm',
  searchResults: 'Treffer',
  rankingTable: 'Turnierrangliste',
  playerNames: 'Spielernamen',
  teamNames: 'Teamnamen',
  matchLabel: 'Begegnung',
  versus: 'gegen',
  setScore: 'Satz',
  moveParticipant: 'Teilnehmenden verschieben',
  moveInstructions:
    'Teilnehmender ausgewählt. Wähle eine andere Verschieben-Schaltfläche, um die Positionen zu tauschen.',
}
